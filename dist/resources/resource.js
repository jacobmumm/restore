...
const findOne_1 = require('../actions/findOne');
const destroy_1 = require('../actions/destroy');
...
class Resource {
    ...
    destroy(payload, config) {
        ...
        return this.$q.when(this.beforeDestroy(id, config))
            .then(args => this.store.dispatch(destroy_1.destroy(this, id, config)))
    ...
    find(args) {
        return this.$q.when(this.beforeFind(args))
            .then(args => this.store.dispatch(find_1.find(this, args)))
            .then(data => this.afterFind(data));
    }
    /**
     * Default identity hook (return what was passed in)
    ...
    findOne(id, config) {
        return this.$q.when(this.beforeFindOne(id, config))
            .then(args => this.store.dispatch(findOne_1.findOne(this, args)))
